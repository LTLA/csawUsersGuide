...
This is true in this particular simulation, where the DB is very strong:
...
Reducing the power results in loss of FDR control, as expected:

```{r}
set.seed(80)
counts2 <- rbind(matrix(rnbinom(n.sites*nlibs, mu=50, size=P), ncol=nlibs, byrow=TRUE),
    matrix(rnbinom(db.sites*nlibs, mu=c(70, 70, 30, 30), size=P), ncol=nlibs, byrow=TRUE))
top.al2.2 <- apply(counts2, 1, FUN=function(x) { sort(x)[nlibs-1] })
keep.al2.2 <- length(top.al2.2) - rank(top.al2.2, ties.method="random") +1 <= 20000
null.al2.2 <- which(keep.al2.2) %in% is.null
y.al2.2 <- DGEList(counts2[keep.al2.2,], lib.size=rep(1e6, nlibs))
y.al2.2 <- estimateDisp(y.al2.2, design)
fit.al2.2 <- glmQLFit(y.al2.2, design, robust=TRUE)
res.al2.2 <- glmQLFTest(fit.al2.2, coef=2)
sig <- p.adjust(res.al2.2$table$PValue, method="BH") <= 0.05
sum(sig & null.al2.2)/sum(sig)
```
...
```{r}
set.seed(1000)
group.z <- rep(LETTERS[1:3], each=2)
nlibs.z <- length(group.z)
design.z <- model.matrix(~group.z)
counts.z <- rbind(
    matrix(rnbinom(n.sites*nlibs.z, mu=50, size=P), ncol=nlibs.z, byrow=TRUE),
    matrix(rnbinom(db.sites*nlibs.z, mu=rep(c(25, 100), c(4, 2)), size=P), ncol=nlibs.z, byrow=TRUE)
    )
```

Loss of error control is subsequently observed with the "at least 2" filter.
This is because variance inflation is negated, without contributing any DB sites to the contrast between the first two groups (thus rendering the FDR irrelevant).

```{r}
top.al2.z <- apply(counts.z, 1, FUN=function(x) { sort(x)[nlibs.z-1] })
keep.al2.z <- length(top.al2.z) - rank(top.al2.z, ties.method="random") +1 <= 10000
summary(which(keep.al2.z) <= n.sites)
y.al2.z <- DGEList(counts.z[keep.al2.z,], lib.size=rep(1e6, nlibs.z))
y.al2.z <- estimateDisp(y.al2.z, design.z)
y.al2.z$common.dispersion
fit.al2.z <- glmQLFit(y.al2.z, design.z, robust=TRUE)
res.al2.z <- glmQLFTest(fit.al2.z, coef=2)
plotAlpha(res.al2.z$table$PValue)
```
...
counts.x <- rbind(matrix(rnbinom(n.sites*nlibs, mu=50, size=P.n), ncol=nlibs, byrow=TRUE),
...
```{r}
y.all <- DGEList(counts.x, lib.size=rep(1e6, nlibs))
