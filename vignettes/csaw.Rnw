...
\section{Scope}
This document describes the \Bioconductor{} package \Biocpkg{csaw} for detecting differential binding (DB) in ChIP-seq experiments.
...
The descriptions in this user's guide are detailed and will explore the theoretical and practical motivations behind each step of a \Biocpkg{csaw} analysis.
While all users are welcome to read it from start to finish, new users may prefer to examine the case studies presented in the \Biocpkg{chipseqDB} package \cite{lun2015from}, which provides the important information in a more concise format.
...
The \Rfunction{filterWindowsProportion} function returns the ratio of the rank of each window to this total, where higher-abundance windows have larger ranks.
...
The \Rfunction{filterWindowsGlobal} function returns the increase in the abundance of each window over the global background.
...
We apply \Rfunction{filterWindowsLocal} to compute enrichment values, i.e., the increase in the abundance of each window over its neighborhood.
...
We use the \Rfunction{filterWindowsControl} function to compute the enrichment of the ChIP counts over the control counts for each window.
...
Any filter statistic for the windows should be based on the average abundance from \Rfunction{aveLogCPM}, such as those calculated using \Rfunction{filterWindowsGlobal} or equivalents.
...
The \Rcode{num.up.tests} and \Rcode{num.down.tests} fields contain the number of windows that change in each direction, 
and can be used to gauge whether binding increases or decreases across the cluster.
A complex DB event may be present if both \Rcode{num.up.tests} and \Rcode{num.down.tests} are non-zero 
...
Some of these events can be identified using the \Rfunction{mixedTests} function.
...
\Rfunction{mixedTests} converts the $p$-value for each window into two one-sided $p$-values.
...
For example, the \Rcode{num.up.tests} and \Rcode{num.down.tests} fields can be used to identify regions with changes in both directions.
...
\subsection{Enforcing a minimal number of DB windows}
...
We can identify these regions using the \Rfunction{minimalTests} function.
...
\Rfunction{minimalTests} applies a Holm-Bonferroni correction to all windows in the same cluster and picks the $x$\textsuperscript{th}-smallest adjusted $p$-value (where $x$ is defined from `min.sig.n` and `min.sig.prop`).
...
If the $x$\textsuperscript{th}-smallest $p$-value is low, this provides strong evidence against the joint null for that cluster.
