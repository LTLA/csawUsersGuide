...
<<>>=
abundances <- aveLogCPM(data$counts, lib.size=data$totals)
length(abundances)
summary(abundances)
@
...
The abundances of the bins can then be estimated using \code{aveLogCPM} and downscaled according to the computed \code{scaled} value.
...
<<>>=
prior.count <- 0.5
win.ab <- aveLogCPM(data$counts, lib.size=data$total, prior.count=prior.count)
bin.ab <- aveLogCPM(binned$counts, lib.size=binned$total, prior.count=prior.count*scaled)
...
This ensures that any enriched regions or binding sites inside the window do not interfere with estimation of its local background.  
