...
Simply filtering out low-abundance windows won't avoid zeros, as the other subtracted counts (e.g., for the replicate) might be large due to variability for high raw counts.
...
Subtraction may be okay if there aren't a lot of regions with high control coverage, as the other high-abundance regions will buffer any damage to the EB statistics (though, if those regions are rare, then there mightn't be much point in correcting anyway).
