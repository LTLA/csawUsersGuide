...
\item Watch out for large peaks, because even in TF data, the subpeaks tend to spread such that you end up with little bimodality.
...
The regularization stuff presented by Wolfgang at Bioconductor 2015 seems to help here.
...
Or, you can think about it by conditioning on cluster formation where each constituent window must pass the filter threshold.
...
The problem is a bit more difficult for the local methods, where we're working with a difference in correlated abundances between each window and its neighbourhood.
Independence between the $p$-value and each abundance is not enough.
Cut up the uniform distribution into four slices; two upper triangles, and two lower triangles. 
...
However, filtering on both statistics yields only one triangle and a non-uniform distribution.
...
See commit \texttt{e8efd8f85ae62428c765716bad97a87ed514572d} of \texttt{master/filtering/filtration.tex} in my thesis.
...
This is because the dispersions will now absorb the batch effect, which will differ randomly between windows.
As a result, the variability in the dispersions will be increased.
There mightn't ba large increase in the dispersions themselves due to a stochastic batch effect, as not all features will be heavily affected by the batch to cause a substantial increase in the fitted trend or the common estimate.
