...
This is important as most windows are low-abundance, which means that the density at the filter boundary is high relative to the number of retained windows.
