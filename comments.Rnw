...
As in \code{treat}, I think that the magnitude of the enrichment over background is more important than the significance of a non-zero enrichment (in fact, we could actually use \code{treat} to get the $p$-value).
