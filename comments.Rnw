...
(Or specifically, the noise around the trend is larger than the range of covariates, making it difficult to pull out a trend.)
\end{itemize}
...
Furthermore, the use of a constant threshold for the p-value focuses on control of type I error and elimination of uninteresting features. 
...
In short, you might get a FDR for your filter, but that doesn't have any effect on FDR control for differential binding.
...
For example, a log-fold change of 2 suggests that you can get, at most, a DB fold change of 2 for your regions that scrape past the filter.
