...
\item It also assumes that the controls are accurate estimators of background, which mightn't be the case, e.g., for inputs.
...
Consider a binding site of width 400 bp, comprised of two Beta(2, 2) subpeaks on either strand of size 200 bp.
