...
The first gene displayed should have no differences between the first two groups upon normalization, but DE in the third group prevents this from happening.
Similarly, the second gene should have no differences between the first and last groups, but this is disrupted by DE in the second group.
...
y <- rbind(y, c(2, 4, 10), c(2, 10, 3), c(2,4,3))	
...
Using true pairwise cyclic normalization fixes the first gene, but the second gene still results in DE.
...
This is not the case for the second gene.
...
Another disadvantage of pairwise normalization is that the covariates are less stable.
