...
\begin{itemize}
\item It obviously requires sequencing of a negative control sample for each condition, which adds to cost.
This may not even be possible for complex designs without a one-way layout.

\item It assumes that the controls are accurate estimators of background, which mightn't be the case, e.g., for inputs.

\item Subtraction will distort the mean-variance relationship as the absolute size of the counts is lost.
...
\item Negative counts need to be coerced to zeros, resulting in an excess of zeros that aren't easily handled by the NB model.
...
\item In practice, other high-abundance regions will buffer any damage to the variance and EB statistics.
...
Conversely, smaller libraries with no undersampling will have more zeroes that get implicitly trimmed, resulting in spurious differences.

\begin{itemize}
\item Adding an pseudo-count doesn't help as the M-value will change dramatically with the value of the pseudo-count when the counts are small.
...
\item Zero counts can only be overcome by incorporating a probability distribution to model its frequency. 
...
A similar problem is encountered with \code{empiricalFDR}.
In this case, an estimate of the number of false positives is obtained by looking at the number of rejections in the wrong direction.
This exploits the fact that the sign is independent of significance under the equality null, so the p-value distribution of false positives in each direction should be equal.
