...
Read stacks can be effectively eliminated by setting a non-negligble threshold on the mapping quality.
I suspect it's because they're formed by repeat regions, and so MAPQ is low due to non-uniqueness.
Alternatively, they might be contaminants that are just trying to find a (poorly aligned) home on the genome build.
Use of \code{discard} to ignore blacklisted regions is less effective as these regions tend not to overlap unannotated repeats.
...
I've dsecribed running TMM on the high-abundance bins, but an equivalent approach is to apply TMM on each window directly. 
I generally don't like doing this as it's harder to assess the context in which the normalization factor is being chosen (and the potential effect of fiddling with the filter threshold). 
The abundance range for window counts is too small for effective visualisation on an MA plot. 
There's also the fact that each read is counted multiple times, though this isn't a real problem.
...
One might think to overcome this problem by using covariates computed in a pairwise manner, as in Affymetrix-based normalization.
This helps a little, but the core of the problem still remains.
Note that this is not truly pairwise normalization, as the adjustment to the values is made after all pairwise normalization is performed.
In true pairwise normalization, adjustments are made immediately after each pairwise comparison.
...
Using true pairwise cyclic normalization fixes the first gene, but the second and third genes still have spurious DE.
...
This is not the case for the second and third genes.
...
The analogy to the normal case suggests that the average abundance is independent of the $p$-values and variances.
This means that it can be used as an independent filter statistic, without affecting EB shrinkage or DB testing.
I guess that the variance is comprised of the QL and NB dispersions.
Estimation of each parameter in \edger{} assumes that the other is known.
If the mean is independent of the variance, then it implies that the mean is independent of each dispersion estimate.
Independence also assumes that the null design matrix can be parametrized with an intercept column.
...
% Cleanest under the null; selection of strong DB peaks might favour selection of other strong DB peaks in other groups that have lower dispersions in the mean-variance trend.
...
This may also be necessary if you don't want to have high spatial resolution, for whatever reason.  
For example, if you just want to find peaks that are just on/off, then you look at the DB for the window with the maximum abundance.
You can then avoid the difficult-to-interpret regions involving complex DB events.
