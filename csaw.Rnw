...
file named \code{xxx.bam}. Users should be aware that the Rsamtools package
(upon which \pkgname{} relies) will not check whether the index is sensible. A
common error is to replace the BAM file without updating the index. This will
cause \pkgname{} to fail silently when it attempts to load reads from the BAM file.
...
fragment corresponding to a read is imputed by directionally extending each
read to the average fragment length. The number of fragments overlapping a
genomic window is counted. This is repeated after sliding the window along the
genome to a new position.  A count is then obtained for each window in each
library. 
...
site may be counted alongside reads from a non-DB site (e.g., non-specific
background) or even those from an adjacent site that is DB in the opposite
direction. This will also result in the loss of DB detection power.
...
setting \code{dedup=TRUE}. This can reduce the variability generated by
...
not recommended for routine DB analyses. That said, removal may not be
avoidable in some cases, e.g., libraries generated from low quantities of DNA.
...
non-uniquely aligned sequences. Removal of these reads is generally recommended
as it will ensure that only the reliable alignments remain in the input to
\pkgname{}.  The exact value of the threshold depends on the range of scores
provided by the aligner. The \subread{} program \citep{liao2013} was used to
align the reads in this dataset so a value of 100 is appropriate.
...
nuclear factors.  Similarly, it allows \code{windowCounts} to work on huge
...
avoid detection of these biologically irrelevant differences. Annotated repeats
identified with the RepeatMasker software can be found for a number of species
on the UCSC website, e.g.,
...
Other potential uses for \code{discard} include the removal of reads
overlapping known binding sites when attempting to discover new sites. This is
safer than simply ignoring windows that overlap the known sites, which would
throw out a window overlapping a known site even if it also contained a new
site. In all cases, any DB sites within the discarded regions will obviously
not be detected during downstream analyses. Some caution is therefore required
when specifying the regions of disinterest.
...
\code{pet="both"} in the \code{windowCounts} function. Read extension is
not required for ChIP-PET data as the genomic interval spanned by the
originating fragment is explicitly defined by the positions of the paired
reads. The number of fragments overlapping each window is then counted as
described. By default, only proper pairs are used whereby the two reads are
on the same chromosome, face inward and are no more than \code{max.frag} apart.
...
the read with the higher mapping quality score will be directionally extended
to \code{ext} to form a fragment. Both reads will be extended in this manner
for inter-chromosomal read pairs. Counting will then be performed with these
fragments in addition to those from the valid pairs. A value of \code{ext} can
be chosen based on the mode of the distribution above.
...
around narrow regions of enrichment, e.g., TF binding sites. The location of
the peak can be used as an estimate of the average fragment length ($\sim$ 110
bp below) for read extension. 
...
default values for most of the critical parameters except \code{bam.files},
\code{ext} and \code{width} (see the call to generate \code{data}, above).
...
be desirable on occassion to use the same conventions (e.g., duplicate
removal, quality score filtering) when counting reads into pre-specified
regions. This can be performed with the \code{regionCount} function, which is
largely a wrapper for the \code{findOverlaps} function from the \granges{}
package.

<<>>=
my.regions <- GRanges(c("chr11", "chr12", "chr15"),
    IRanges(c(75461351, 95943801, 21656501), c(75461610, 95944810, 21657610)))
...
a larger effective library size. This is conceptually equivalent to scaling
each individual count downwards, as the ratio of that count to the library size
will be smaller.  Check out the user's guide in \edger{} for more detail.

Note that \code{normalizeChIP} skips the precision weighting step in the TMM
method. Weighting increases the contribution of bins with high counts.
However, these bins are more likely to contain binding sites and thus are more
likely to be DB. If any DB regions should survive trimming (e.g. those with
less extreme fold changes), upweighting them would then be counterproductive.
In fact, users may wish to explicitly filter out such bins and run TMM only on
putative background regions, as shown below.
...
Binning is necessary to increase the size of the counts for background regions
prior to TMM normalization. This avoids zero counts that lead to undefined
M-values. Adding a prior count is only a superficial solution as the chosen
...
bins or windows with highest abundances contain binding sites. The TMM method
can then be applied to eliminate systematic differences in the counts across
those bins. In the example below, the top 1\% of bins are assumed to contain
binding sites.
...
uninteresting technical issue that has the same effect). Genuine biological
differences may be removed when the assumption of a non-DB majority does not
hold, i.e., overall enrichment is truly lower for one condition. Also, the
percentage of bins to use for normalization may not be obvious if there is no
obvious demarcation on a MA plot.
...
whether one assumes that the differences are genuine or not.
...
me.base <- normalizeChIP(me.demo$counts, lib.sizes=me.demo$totals)
...
ac.base <- normalizeChIP(ac.demo$counts, lib.sizes=me.demo$totals)
...
trend. This may be due to the fact that changes in IP efficiency will have
little effect at low abundances (e.g., background regions) and more effect at
high abundances (i.e., actual binding sites). Thus, the magnitude of the bias
between libraries will change with abundance. The trend cannot be corrected
with scaling methods as no single scaling factor will remove differences at all
abundances.  Rather, non-linear methods are required such as cyclic loess or
...
will act as a vertical cutoff in the plots below.  This avoids introducing any
...
Note that all non-linear methods assume that most bins/windows are not DB for
each point in the covariate range. This tends to be a stronger assumption than
that for for scaling methods, which only require a non-DB majority across all
bins. Removal of the trend may not be appropriate if it represents some genuine
...
for some datasets. This will decrease the observed variability by removing
systematic differences between replicates. Of course, this also assumes that
the targeted differences have no biological relevance.  Detection power may be
lost if this is not true. For example, differences in the GC content
distribution can be driven by technical bias as well as biology, e.g., when
protein binding is associated with a specific GC composition.
...
counts will not provide enough evidence against the null hypothesis to obtain
low $p$-values. Similarly, some statistical approximations will fail at low
counts.  Removing such uninteresting tests reduces the severity of the multiple
testing correction, increases detection power amongst the remaining tests and
...
this corresponds to filtering on the overall NB mean. The DB $p$-values
retained after filtering should then be uniform, assuming analogy behaviour to
the normal case. Row sums can also be used for datasets where the effective
library sizes are not very different or where the counts are assumed to be
Poisson-distributed between biological replicates.
...
retain enough points for successful information sharing in
Chapter~\ref{chap:stats}.
...
adapt well to different datasets where the proportion of binding can vary.
Assuming an inappropriate percentage of binding will result in loss of
...
comparison to windows. If one assumes a uniform distribution of reads in
background regions, the threshold can be directly scaled down based on
differences in the size of the read counting interval between windows and bins.
For windows with \code{ext > 0}, the size of the interval is equal to \code{ext
+ width} due to strand-specific read extension.
...
global background. Here, a fold change of 10 over the background is necessary
for a window to be considered as containing a binding site.  This approach has
an intuitive and experimentally relevant interpretation which adapts to the
level of non-specific enrichment in the dataset. 
...
sites will have high read densities and are unlikely to be filtered out.
Smaller minimum fold changes are recommended for regions of diffuse enrichment
where the difference to background is less obvious. 
...
similar size or biological variability is low, reads can be pooled into a
single library for single-sample peak calling. This is equivalent to filtering
on the average count and avoids distortion of the type I error rates due to
data snooping.
...
sequencing and mapping biases in ChIP-seq data. IgG controls also provide a
measure of non-specific enrichment over the genome. These libraries are largely
irrelevant when testing for DB. However, they can be used to filter out windows
with an average count in the ChIP sample below that of the control.  The
example below requires a 5-fold or greater increase over the control to retain
the window.
...
windows. Overlapping can also be combined with abundance filtering to obtain
windows that contain binding sites in the regions of interest.
...
non-linear normalization), this can be assigned into \code{y\$offset} for use
...
downstream statistical processing. Specifically, the same library sizes should
be used throughout all steps.  The \code{totals} vector is typically used as it
is constant for all bin sizes with any given dataset/read extraction
parameters. Simply taking the column sums is usually inappropriate due to
overlaps between windows and filtering.
...
from the Poisson and Gamma components. The latter accounts for variability and
...
for precise estimation of either dispersion. 
...
reduced accuracy in the fitted values of the trend, typically manifesting as
overestimation of dispersions at high abundances. Users should check whether
removal of the low abundance regions affects the dispersion estimate. Large
differences imply that further abundance-based filtering may be desirable, 
as described in Chapter~\ref{chap:filter}.
...
heteroskedasticity is low. More EB shrinkage can be performed to reduce
...
null hypothesis as \code{0*intercept + 1*cell.type = 0}, i.e., that the log-fold
change between cell types is zero. DB windows can then be identified by rejecting
the null. Specification of the contrast is explained in greater
...
of the genome in which (differential) protein binding is found, rather than the
individual windows. In other words, the FDR across all detected DB regions
is usually desired. This is not equivalent to that across all DB windows as each
region will often consist of multiple overlapping windows. Control of one will
not guarantee control of the other \citep{lun2014}.
...
function from the \granges{} package can be used to identify all windows in or
...
background regions. In both cases, window-based methods are more robust as each
interval of the promoter/peak region is examined separately.
...
Like before, the row names in the output table point towards the corresponding
coordinates of the clusters in \code{merged\$regions}. This allows for 
simple correspondence between the results and the regions. 
...
This can viewed as the maximum distance at which two binding sites are 
...
opposite directions. A vector cam ne supplied in \code{sign} to indicate
...
This results in more large $p$-values and conservativeness. Furthermore, any
attempt to filter away small clusters will cause liberalness if too many large
$p$-values are lost.
