...
The function returns a \code{SummarizedExperiment} object where the matrix of counts is stored as the first entry in the \code{assays} slot.
...
This ensures consistency with some downstream functions, which assume that the \code{ext} field of the returned \code{SummarizedExperiment} holds the average fragment length.
...
We'll also need the \code{aveLogCPM} function from \edger{}, so load the package if you haven't already.
\end{combox}
...
<<results=hide,echo=FALSE>>=
rm(binned, in.demo, wider, neighbour, chip, control, filter.stat)
gc()
@
...
Replicates from different groups should form separate clusters in the MDS plot.
This indicates that the results are reproducible and that the effect sizes are large.
Such clustering is observed in the example above, along with separation from the input sample (note that the input is not essential for plotting, and is only included for demonstration purposes).
...
<<results=hide,echo=FALSE>>=
rm(binned, demo, ac.demo, ac.bin, me.demo, me.bin, ac.demo2, filtered, keep, adj.counts, adjc)
gc()
@
...
\begin{combox}
Here, we'll need the \code{filtered.data} from Chapter~\ref{chap:filter} and the original \code{data} from Chapter~\ref{chap:count}.
We'll also need the \code{normfacs} vector from Chapter~\ref{chap:norm}, as well as the \code{design} matrix from the introduction. 
...
Large values (500 - 1000 bp) reduce redundancy and favour a region-based interpretation of the results, while smaller values ($< 200$ bp) allow resolution of individual binding sites.
...
The use of the highest-abundance window will also favour detection of simple DB events, where DB occurs at the most strongly bound loci in each cluster.
This may be preferable for some studies, in order to avoid complex DB that is difficult to interpret.
...
extractReads(cur.region, bam.files[1], param=param)
@
...
for (i in 1:length(bam.files)) { 
    reads <- extractReads(cur.region, bam.files[i], param=param)
