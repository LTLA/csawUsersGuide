...
This means that the same reads are probably extracted from the BAM files in each call. 
...
Observant readers may notice that \code{ext} was specified in the previous call to \code{windowCounts}.
...
This ensures consistency with some downstream functions, which assume that the \code{ext} field of the returned \code{SummarizedExperiment} object holds the average fragment length.
...
This value is useful as it focuses on the DB interval within the cluster.
Taking the average log-fold change across all windows in a cluster will understate the magnitude of the change.
