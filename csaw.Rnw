...
If an offset matrix was computed, it can be assigned to \code{y\$offset}, in which case it will be used in favour of normalization factors in the \edger{} functions.
(Alternatively, users can store normalization information directly in the \code{RangedSummarizedExperiment} object, which simplifies data management.
This information will then be automatically extracted and used to construct the \code{DGEList} object -- see \code{?asDGEList} for more details.)
