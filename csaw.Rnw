...
par(mfrow=c(1,2))
collected.compo <- list()
for (main in c("H3K4me3", "H3ac")) { 
    if (main=="H3K4me3") { demo <- me.demo; norm <- me.norm }
    else { demo <- ac.demo; norm <- ac.norm }
    ...
    collected.compo[[main]] <- compo.fac
}
@
...
For the sake of comparison, the sizes of the normalization factors for composition bias can also be examined.
These will generally scale the library sizes in the opposite direction to the factors for efficiency bias.
This is because any increase in IP efficiency in one library will lead to greater suppression of background regions in the same library.
