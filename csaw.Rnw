...
This is computed with the \code{aveLogCPM} function.
In \pkgname{}, the average abundance is further adjusted for the size of the windows using the \code{apkb} function.
This reports the average abundance-per-kilobase and allows comparison of abundances between regions of different sizes.

<<>>=
abundances <- apkb(data)
summary(abundances)
@

For demonstration purposes, an arbitrary threshold of 0 is used here to filter the window abundances. 
...
The median of the binned abundances-per-kilobase can be used as a global estimate of the background coverage.
...
The width of the window is also subtracted to obtain the effective size of the neighbourhood.
These can then be supplied to \code{apkb}, as shown below.
...
% A larger prior count could be used here, as each estimate of the local background is less stable than a single global value.
% However, this would require that the window abundances-per-kilobase need to be recomputed with the new prior.

<<>>=
assay(wider) <- assay(wider) - assay(data)
neighbour.width <- width(neighbour) - width(rowData(data)) 
neighbour.ab <- apkb(wider, widths=neighbour.width)
@
...
For sharp binding events, it may be useful to restrict the analysis to these windows.
This is slightly easier to interpret and avoids the conservativeness of Simes' method.
However, multiple DB events in a cluster will not be reliably identified, e.g., multiple adjacent TF binding sites.
...
This example finds the window with the largest absolute log-fold change in each cluster.
