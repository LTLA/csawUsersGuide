...
Finally, it is possible to develop a more sophisticated background-based filter where each window is tested against a null background model.
...
% There's problems with computing the p-values and correcting for multiple tests, but we'll ignore those for now.
...
% Another problem is that you're requiring rejection at an FDR of 5% in order to get into the retained set.
...
%
% In short, you can either get the minimum log-FC or FDR for the selected set, but neither tells us much about how our DB analysis will turn out.
% The FDR does give us something in terms of the range of possible BH correction values, but this is pretty wide (as it depends on how many DB things are there).
% I'm not sure that the extra work is worth this little information.
