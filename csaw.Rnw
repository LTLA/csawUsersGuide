...
lines(collected[[3]], col=rgb(0,0,0,0.5), lwd=2)
@
...
The ideal increase in the window size can be defined as twice the distance at which the surrounding coverage becomes negligble.
In the example above, a window size of $\sim$1 kbp may be most appropriate for the two histone marks.
This corresponds to a distance of 500 bp, which captures most of the additional coverage without including too much background contamination.
