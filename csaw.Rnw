...
<<>>=
require(edgeR)
keep <- aveLogCPM(assay(data), lib.size=data$totals) >= -1
data <- data[keep,]
@
\item Identifying DB windows.
<<>>=
y <- DGEList(assay(data), lib.size=data$totals, norm.factors=normfacs)
...
Sorting should be performed on read position. 
Each index file should be named as \code{xxx.bam.bai} for the BAM file named \code{xxx.bam}. 
...
Each row corresponds to a genomic window whereas each column corresponds to a library.
...
On a more subjective note, reads on particular chromosomes can be specifically counted by specifying the chromosomes of interest in \code{restrict}. 
...
Finally, reads lying in certain regions can also be removed by specifying those regions in \code{discard}. 
...
Annotated repeats can be found for a number of species on the UCSC website, e.g., \href{hgdownload.soe.ucsc.edu/goldenPath/mm10/bigZips/chromOut.tar.gz}{mouse}.
...
Users are encouraged to construct their own \code{readParam} objects, for consistent application throughout their analyses.
...
computed by \code{aveLogCPM}/\code{average} is strongly recommended as the filter statistic
...
<<>>=
prior.count <- 0.5
win.ab <- average(data, prior.count=prior.count)
bin.ab <- average(binned, prior.count=prior.count*scaled)
...
<<>>=
surrounds <- 2000
neighbour <- suppressWarnings(resize(data$region, surrounds, fix="center"))
wider <- regionCounts(bam.files, regions=neighbour, ext=frag.len)
@

On a side note, if a non-default \code{param} was used in \code{windowCounts}, users should ensure that the same value is passed to \code{regionCounts} as well.
This ensures that the same reads are used for window- and region-based counting.
...
prior.count <- 1
win.ab <- average(data, prior.count=prior.count)
neighbour.ab <- aveLogCPM(neighbour.counts, lib.size=wider$totals, prior.count=scaled*prior.count)
...
chip <- average(in.demo[,1:4])
control <- average(in.demo[,5])
