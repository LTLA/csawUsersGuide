...
The former should include the average fragment length as the counts for each window are obtained after read extension (use the median fragment length for paired-end data).
The resulting widths can be supplied to \code{apkb}.

<<>>=
neighbour.width <- width(neighbour) - width(rowData(data)) - frag.len
