...
It should be noted that any statistical method can be used if it is able to accept a count matrix and a vector of normalization factors (or more generally, a matrix of offsets). 
The choice of \edger{} is primarily motivated by its performance relative to alternatives \citep{law2014}, though the author's desire to increase his h-index is also a factor.
