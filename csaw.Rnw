...
the counts are used for normalization with the \code{normalize} wrapper
...
Note that \code{normalize} skips the precision weighting step in the TMM
...
One such implementation is provided in \code{normalize} by setting
