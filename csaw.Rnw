...
Conversely, \code{spacing} should not be larger than \code{max(width, ext/2)} for analyses with small windows.
...
Most values will lie somewhere above zero, given that some pre-filtering has already been performed on the window counts during loading in \code{windowCounts}.
...
chip <- in.demo[,1:4]
control <- in.demo[,5]
keep <- filterWindows(chip, control, type="control")$filter > log(5)
@
