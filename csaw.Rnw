...
% The obvious case is when ext=1, where you would lose reads when windows are shifted. 
...
In the example above, most enrichment for the two histone marks is focused within a 350 bp region around the maxima.
...
The strand of the output \code{rowData} indicates the strand on which reads were counted for that row.
...
This can be achieved by assuming that high-abundance bins contain binding sites. 
In the example below, these bins are chosen based on the approach described in Section~\ref{sec:global_filter}. 
...
keep <- filterWindows(me.demo, type="global")$filter > log2(10)
me.norm <- normalize(me.demo[keep,])
me.norm

ac.demo <- windowCounts(c("h3ac.bam", "h3ac_2.bam"), bin=TRUE, width=10000L)
keep <- filterWindows(ac.demo, type="global")$filter > log2(10)
ac.norm <- normalize(ac.demo[keep,])
ac.norm
@
\label{data:norm}

Note that the bin size in Section~\ref{sec:compo_norm} is only used here for consistency (and to allow comparison between the different normalization methods on a MA plot, later).
The counts for binding sites should be high enough such that smaller bins can be used.
TMM normalization can even be applied directly to the high-abundance windows of interest.
This ensures that there is no systematic difference between libraries across these windows.
...
In that respect, any filtering that needs to be done (see Chapter~\ref{chap:filter}) should be carried out \textit{before} this normalization step.
The example below operates on the filtered counts for small 2 kbp bins. 

<<>>=
ac.demo2 <- windowCounts(c("h3ac.bam", "h3ac_2.bam"), width=2000L)
filtered <- filterWindows(ac.demo2, type="global")
keep <- filtered$filter > log2(3)
...
This requires some care as the sizes of the regions used for read counting are different.
The average abundance of each bin must be scaled down to be comparable to those of the windows.
The median of the adjusted binned abundances is then used as a global estimate of the background coverage.
With \code{type="global"}, the \code{filterWindows} function returns the increase in the abundance of each window over the global background estimate.
...
abline(v=median(filter.stat$back.abundances)+log2(3), col="red")
@
...
% A larger prior count of 5 is also used here, compared to the default value of 2.
% This avoids situations where a window has high enrichment simply because very few counts are observed in its local background.
% Otherwise, you might end up selecting for low-abundance windows.
...
Most values will lie somewhere above zero, given that some pre-filtering has already been performed to select for higher-abundance windows during counting in \code{windowCounts}.

<<eval=FALSE,label=enriched>>=
enriched.fc <- filter.stat$filter
hist(enriched.fc, xlab="Enrichment log-FC")
@
...
The dummy example below requires a 3-fold or greater increase over the control to retain the window.

<<>>=
in.demo <- windowCounts(c(bam.files, "IgG.bam"), ext=frag.len)
chip <- in.demo[,1:4]
control <- in.demo[,5]
keep <- filterWindows(chip, control, type="control")$filter > log2(3)
@
...
% This is the best way to deal with GC biases, as you can just grab an input off the web and use it (citation?).
% You could do so with your existing ChIP data by constructing a GC-abundance curve, and then correcting your window abundances based on the fitted value.
% However, that gets messed up if you have systematic genuine binding at any given GC content, which will kick up the curve.
...
Rather, these filters provide a relevant interpretation for what would otherwise be an arbitrary threshold.

As a general rule, users should filter less aggressively if there is any uncertainty about the features of interest.
This means that more potential DB windows are retained for testing.
...
Finally, it is possible to develop a more sophisticated filter where each window is tested against a null background model.
Windows with low $p$-values can be selected to remove background regions.
In practice, the absolute size of this $p$-value tends to be difficult to interpret, especially once multiplicity corrections are applied.
The philosophy of background-based filters in \pkgname{} is to use the log-fold increase relative to the background estimate.
This is easier to compute, and the threshold is easier to pick and adjust for users.

% To compute a p-value, you'd need to work with the overall NB mean, not the raw counts, to avoid any lack of independence.
% The advantage of putting a threshold on the p-values is that the filtering can account for changes in variance between datasets (also, we can avoid prior.counts).
% The question is, how do you get this variance?
% The gold standard would be to do repeated controls, and use the variance estimate between controls, but that's expensive.
% Alternatively, you could use a global dispersion estimate across all windows in the same dataset.
% This will increase p-values in datasets with more variability in the background, so filtering becomes more aggressive for both background and genuine regions.
% For example, if the distribution of background abundances spreads enough to cover the distribution for genuine binding sites, you'd start losing the latter.
% Finally, if you go Poisson, the variance function is constant with a global lambda, so you'd select the same windows as with a log-FC threshold across datasets.
% For a local lambda, you'd end up picking high-abundance windows with low log-FC's that won't be significant in the NB model.
...
tabcom$best.start <- start(rowData(data)[tab.best$best])
...
% This may be necessary if you don't want to have high spatial resolution, for
% whatever reason.  For example, if you just want to find peaks that are just
% on/off, then you look at the DB for the window with the maximum abundance.
% This still has some advantages over straight-up peak calling, as you still
% maintain high resolution, i.e. you don't get contamination from miscalled
% peak boundaries.
...
weights[tab.ave$best] <- freq[merged$id[tab.ave$best]] 
...
% To wit; let 'w' be the weight of the highest abundance window, and N be the number of windows in a cluster.
% If the highest abundance window is the most DB, then the penalty is (N+w-1)/w, i.e., the multiplicity correction compared to just using the p-value of that window.
% If another window is most DB, then the penalty is (N+w-1)/N, i.e., compared to unweighted Simes.
% These two penalties can be made equal by setting w=N, which means the penalty is just below 2.
