...
file named \code{xxx.bam}. Users should be aware that the sensibility of the 
index is not checked prior to counting. A common mistake is to replace or update
the BAM file without updating the index. This will cause \pkgname{} to return
incorrect results when it attempts to load alignments from new BAM file.

\section{Counting reads into windows}

\subsection{Overview}
The \code{windowCounts} function counts fragments for a set of libraries using
a sliding window approach. For single-end data, the fragment corresponding to a
read is imputed by directionally extending each read to the average fragment
length. The number of fragments overlapping a genomic window is counted. This
is repeated after sliding the window along the genome to a new position.  A
count is then obtained for each window in each library. 
...
setting \code{dedup=TRUE}. This can reduce the variability introduced by
...
not recommended for routine DB analyses. Of course, removal may not be
avoidable in some cases, e.g., involving libraries generated from low
quantities of DNA.
...
discarded regions will obviously not be detected during downstream analyses.
...
multiple calls to the \code{windowCounts} function. Users should supply the
same values for the \code{bam.files}, \code{dedup}, \code{pet}, \code{minq},
...
\code{windowCounts} function. This performs read counting with the default
values for most of the critical parameters except for \code{bam.files},
...
a larger effective library size and is conceptually equivalent to scaling
each individual count downwards (as the ratio of that count to the library size
will be smaller). Check out the \edger{} user's guide for more information.
...
contain binding sites. For consistency, the same bin size is used here though
the counts for binding sites should be high enough for smaller bins.
...
This method assumes that most high-abundance bins are not DB. Any changes must
be due to differences in IP efficiency, or caused by some other uninteresting
technical issue. Genuine biological differences may be removed when the
assumption of a non-DB majority does not hold, i.e., overall enrichment is
truly lower for one condition. Also, the percentage of bins to use for
normalization may not be obvious if there is no obvious demarcation on a MA
plot.
...
\citep{ballman2004} with minor adaptations to count data. It can be applied
after some filtering to remove low-abundance regions where the loess curve
fitting is not accurate. An offset matrix is then produced that contains the
log-effective library sizes for each supplied bin/window. This can be supplied
to the statistical methods in \edger{}, assuming that said bins or windows are
also the ones to be tested for DB. The example below performs non-linear
normalization on the counts for smaller bins, of which the top 5\% are assumed
to contain binding sites.
...
This is because an average count threshold will act as a vertical cutoff in the
plots below. Thus, spurious trends that might affect normalization are not
introduced.
...
provide enough evidence against the null hypothesis to obtain low $p$-values.
Similarly, some statistical approximations will fail at low counts.  Removing
such uninteresting or ineffective tests reduces the severity of the multiple
...
$p$-values retained after filtering should then be uniform, assuming analogous
behaviour to the normal case. Row sums can also be used for datasets where the
effective library sizes are not very different or where the counts are assumed
to be Poisson-distributed between biological replicates.
...
and bins. For counts obtained with read extension, the size of the interval is
equal to \code{ext + width}. Only one \code{ext} is added because expansion of
the interval by read extension is strand-specific, i.e., only forward-mapping
reads are counted on the left of the window and only reverse-mapping reads are
...
filtering.  Smaller minimum fold changes are recommended for regions of diffuse
enrichment where the difference to background is less obvious. 
...
with an average count in the ChIP sample below that of the control.  The dummy
...
Overlapping can also be combined with abundance filtering to obtain windows
that contain binding sites in the regions of interest.
...
independent annotation can be used without too many problems. However, using DB
calls from the same dataset to filter regions would violate the null assumption
...
throughout all steps.  The \code{totals} vector is typically used as it is
constant for all window/bin sizes with any given dataset/read extraction
parameters. Simply taking the column sums is usually inappropriate due to
overlaps between windows and the effects of filtering.
...
dispersion estimate. Large differences imply that further abundance-based
filtering may be desirable, as described in Chapter~\ref{chap:filter}.
...
\code{windowCounts}. Thus, it can be used to ensure that the visualization
