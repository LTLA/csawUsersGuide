...
A simple strategy for normalization involves counting reads into large bins, which are used in \code{scaleControlFilter} to compute a normalization factor.
...
This uses the trimmed mean of M-values (TMM) method \citep{oshlack2010} to correct for any systematic fold change in the coverage of the bins. 
...
With \pkgname{}, separate calls to \code{windowCounts} with the same \code{readParam} object will always yield the same library sizes in \code{totals}.

\paragraph{What is \code{normOffsets} actually doing?}
The \code{normOffsets} call computes normalization factors from \code{binned} and returns a modified version of \code{filtered.data}.
...
We use \code{se.out=FALSE} to instruct \code{normOffsets} to return the normalization factors directly.
This is more convenient than returning a \code{RangedSummarizedExperiment} object and extracting the normalization factors with \code{\$norm.factors}.
...
Here, the binned counts in \code{me.bin} are only used for defining the background abundance, \emph{not} for computing normalization factors.
...
Only \code{normOffsets} is actually performing the normalization step.
We set \code{se.out=TRUE} so that the normalization factors are stored alongside the window counts for use in the downstream \edger{} analysis.
...
Assuming that the reference genome includes appropriate sequences from the foreign genome, coverage is quantified across genomic windows with \code{windowCounts}.
...
Normalization factors are computed by applying the TMM method on these counts via \code{normOffsets}.
...
\paragraph{What is the \code{normOffsets} call doing?}
The spike-in normalization factors are computed using high-abundance regions in \code{spike.data}, and returned in a modified copy of \code{endog.data}.
The new \code{endog.data} can then be used for further analysis of the endogenous windows.
The normalization factors can also be extracted from \code{endog.data\$norm.factors}, as previously described.
We assume that the library sizes in \code{totals} are the same between \code{spike.data} and \code{endog.data}, 
which should be the case if they were formed by subsetting the output of a single \code{windowCounts} call.
...
When \code{se.out=TRUE}, the offsets are stored in the \code{RangedSummarizedExperiment} object as an extra entry in the \code{assays} slot.
...
If the offsets are available, they will generally override the normalization factors from the scaling methods in the downstream \edger{} analysis.
...
Any filter statistic for the windows should be based on the average abundance from \code{aveLogCPM}, such as those calculated using \code{filteWdinows}
...
A \code{DGEList} object is first constructed from the count matrix in \code{filtered.data}.
If normalization factors or offsets are present in the \code{RangedSummarizedExperiment} object -- see Chapter~\ref{chap:norm} -- they will automatically be extracted and used to construct the \code{DGEList} object.
...
These commands are stored in a Bash file named \code{sra2bam.sh} at \url{https://github.com/LTLA/csawUsersGuide}.
