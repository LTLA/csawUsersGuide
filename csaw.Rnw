...
Thus, TF analyses will typically use a window size of several base pairs. 
...
Coordinates of annotated repeats can be found for a number of species on the UCSC website, e.g., \href{hgdownload.soe.ucsc.edu/goldenPath/mm10/bigZips/chromOut.tar.gz}{\texttt{mouse}}.
However, this requires some effort to parse.
If possible, the simplest approach is to extract them from an appropriate \code{BSgenome} object. 
The example below takes the repeats called by the RepeatMasker program in the hg19 build of the human genome.
Blacklisted regions from the ENCODE project are another option \citep{landt2012}.
...
The \code{getPETSizes} function also returns the number of single reads, pairs with one unmapped read, improperly orientated pairs and inter-chromosomal pairs.
...
The coverage profile around potential binding sites can be obtained with the \code{profileSummit} function.
This defines ``summits'' as local maxima in the read density across the genome.
The coverage of the surrounding regions is recorded as a function of the distance from the summit.
This is averaged across all summits to obtain a coverage profile for each library.

<<label=profiling,eval=FALSE>>=
collected.ac <- profileSummit("h3ac.bam", param=dedup.on, min.depth=0.5)
collected.me2 <- profileSummit("h3k4me2.bam", param=dedup.on, min.depth=0.5)
collected.tf <- profileSummit("es_1.bam", param=dedup.on, min.depth=0.5)

plot(collected.ac$span, collected.ac$coverage, type="l", col="blue", 
    xlim=c(0, 5000), lwd=2, xlab="Window size", ylab="Relative coverage")
lines(collected.me2$span, collected.me2$coverage, col="darkgreen", lwd=2)
lines(collected.tf$span, collected.tf$coverage, lwd=2)
@
...
The window size can then be identified as the distance at which the surrounding coverage becomes negligble.
In the example above, a window size of around 500 - 1000 bp may be appropriate for the H3 acetylation and H3K4me2 datasets.
This captures most of the additional coverage around the summit without risking contamination from non-enriched background.
...
Indeed, summits are unlikely to be well-defined within diffuse regions.
Users are advised to exercise caution when deciding on an appropriate window size.
...
See Chapter~\ref{chap:filter} for more details on selection of enriched regions.
...
In that respect, any filtering that needs to be done (see Chapter~\ref{chap:filter}) should be carried out \textit{before} normalization.
...
In \edger{}, the log-transformed overall mean is defined as the average abundance.
This is computed with the \code{aveLogCPM} function, as shown below.
Note that the name is a bit of a misnomer - \code{aveLogCPM} computes the logarithm of the overall mean count (expressed as a per-million value), \textit{not} the mean of the log-count-per-million (CPM) values.
...
Users are directed to choose one of these filtering approaches for their own analyses.
...
Under a model of background enrichment, assume that reads are evenly distributed between strands and uniformly distributed across large genomic regions.
Downscaling can then be directly performed based on differences in the size of the read counting interval between windows and bins.
For windows with read extension, the size of the interval is equal to the sum of the window width and the fragment length.
The fragment length is only added once because extension is strand-specific, i.e., only forward- and reverse-mapping reads are counted on the left and right of the window, respectively.
Thus, only half of the reads are counted on the flanks of the extended interval.
...
The abundances of the bins are estimated and downscaled according to the computed \code{scaled} value.
...
However, if the same prior is used for both bin and window counts, downscaling will reduce the effective prior added to the former.
...
This can be synchronized with \code{windowCounts} by using the same \code{param}, if any non-default settings were used.
...
A larger prior count is used here as each estimate of the local background is less stable than a single global value.
Again, scaling of the prior is necessary when computing neighbourhood abundances.
This avoids spurious differences with the window abundances at low counts.
...
In this scenario, a 5-fold increase in enrichment over the neighbourhood abundance is required for retention of each window.
...
Windows can be clustered together into regions, using a number of different strategies. 
...
\section{Getting the most DB window}
In some cases, it may be necessary to also report the window in which the strongest DB is found.
This information can be useful for identifying the relevant window in large complex clusters, or to narrow down the relevant sequence for motif discovery.
...
<<>>=
best <- getBestTest(merged$id, results$table)
head(best)
@

In most cases, the cluster coordinates will be precise enough so that the best window need only be reported as a descriptive measure.
However, more statistical rigour is necessary if the latter are to be reported directly, i.e., as the features of interest over which the error rate is to be controlled.
To maintain error control, a Bonferroni correction is applied to the $p$-value of each best window to obtain the corresponding \code{PValue} in \code{best}.
...
As the average abundance is independent of the $p$-value, no multiple testing correction is necessary under this definition.

<<>>=
best.2 <- getBestTest(merged$id, results$table, mode="logCPM")
head(best.2)
@

Arbitrary summarization schemes can be implemented manually with the \code{aggregate} function.
For example, the code below finds the window with the largest absolute log-fold change in each cluster.

<<>>=
manual <- aggregate(results$table$logFC ~ merged$id,  
...
For the one paired-end library, users will need to specify \code{fastq-dump --split-files} to ensure that two separate files are produced, i.e., one containing sequences from each end.
