...
% Technically, you should also lop off 50 bp, because the loss of resolution inflates the apparent width.
% Imagine if binding occured at 1 bp; random placement within the 50 bp bin would give a summit width of 50 bp.
% However, it's all fairly approximate anyway, so I won't make too much of an issue about it.
% I would only bother if I were to automate this step with another function, otherwise it's too much detail for the user.
