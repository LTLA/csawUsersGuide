...
Some protection is provided by scaling the extended reads to the same length in all libraries.
...
Similarly, all subpeaks on the reverse strand are centered at another constant location across libraries.
This removes most of the differences in width and coverage between libraries.
By default, the mean of \code{ext} is used as the standard extension length, although another value can also be specified.
