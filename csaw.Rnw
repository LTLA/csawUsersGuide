...
However, this approach depends on disk storage systems that support concurrent read access.
Parallelization also has a computational overhead and may be less useful for fewer files.
By default, it is turned off by setting \code{BPPARAM} to a \code{SerialParam} object.
