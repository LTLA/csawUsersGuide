...
\subsubsection{Based on the contribution to the combined $p$-value}
...
This allows the dominant DB in the cluster to be quickly summarized, as shown below for significant regions at a FDR threshold of 5\%.
...
The same approach can be applied for the gene-based clusters.
...
Mixed clusters represent complex DB events involving changes in binding beyond simple increases or decreases in intensity.
This includes regions with multiple DB peaks changing in opposite directions, or peaks that change in shape or position between conditions.
Users should be aware that regions labelled as \code{"up"} or \code{"down"} may also be complex events,
but are not labelled as such due to the fact that the significance calculations are dominated by windows changing in one direction.
...
This allows investigators to easily identify the DB subinterval within large clusters, or to narrow down the relevant sequence for motif discovery.
...
The \code{upweightSummits} function is then used to compute window-specific weights.
These can be supplied to \code{combineTests} for consideration when computing combined $p$-values.
