...
As before, consolidation can be performed by storing these statistics in the per-element metadata of the \code{GRanges} object.
This is often convenient as it synchronizes the processing of the statistics and genomic coordinates.
