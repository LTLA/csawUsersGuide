...
The normalization factors computed from the bin-level counts are then applied to the window-level counts, which is permissible \emph{as long as the library sizes are the same}.
...
<<>>=
data.spnorm <- normOffsets(spike.data, lib.sizes=data$totals, se.out=data)
@
