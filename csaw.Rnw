...
Sorted and indexed BAM (i.e., binary SAM) files are required as input into the read counting functions in \Biocpkg{csaw}. 
Sorting should be performed on the genomic position of the mapped read.
For a given BAM file named \file{xxx.bam}, the corresponding index file should be named as \file{xxx.bam.bai} such that both files are in the same directory. 
...
This will cause \Biocpkg{csaw} to return incorrect results when it attempts to load alignments from new BAM file.
...
For single-end data, suitable values for the average fragment length in \Rcode{ext} can be estimated from the primary peak in a cross-correlation plot (see Section~\ref{sec:ccf}). 
...
The specified \Rcode{width} of each window controls the compromise between spatial resolution and count size. 
...
The idea is to define the \Rclass{readParam} object once in the entire analysis pipeline.
It can then be reused for all relevant functions, thus ensuring that read loading is consistent throughout the analysis.
...
Using \Rcode{discard} is safer than simply ignoring windows that overlap the repeats. 
...
Users are encouraged to construct their own \Rclass{readParam} objects (or lists thereof) and apply them consistently throughout their analyses.
...
For analyses with large windows, it is worth increasing the \Rcode{spacing} to a fraction of the specified \Rcode{width}. 
...
Windows that are overlapped by few fragments can be filtered out using the \Rcode{filter} argument. 
...
Specifically, any window is removed if the sum of counts across all libraries is below \Rcode{filter}.
...
Setting \Rcode{bin=TRUE} will cause \Rfunction{windowCounts} to count reads into contiguous bins across the genome.
Specifically, \Rcode{spacing} is set to \Rcode{width} such that each window forms a bin.
...
As a result, users should set \Rcode{width} to a reasonably large value.
...
ChIP experiments with paired-end sequencing can be accomodated by setting \Rcode{pe="both"} in the \Robject{param} object supplied to \Rfunction{windowCounts}. 
...
A suitable value for \Rcode{max.frag} can be chosen by examining the distribution of fragment sizes from the \Rfunction{getPESizes} function. 
...
Note that all of the paired-end methods in \Biocpkg{csaw} depend on the synchronisation of mate information for each alignment in the BAM file. 
...
Cross-correlation plots can be generated directly from BAM files using the \Rfunction{correlateReads} function. 
...
The location of the peak can be used as an estimate of the fragment length for read extension in \Rfunction{windowCounts}. 
An estimate of $\sim$110 bp is obtained from the plot above.
This can be done more precisely with the \Rfunction{maximizeCcf} function, which returns a similar value.
...
Thus, an \Rcode{ext} list should only specified for datasets that exhibit large differences in the fragment sizes.

\section{Choosing an appropriate window size}
The coverage profile around potential binding sites can be obtained with the \Rfunction{profileSites} function.
Binding sites are defined by taking high-abundance 50 bp windows and identifying those that are locally maximal using \Rfunction{findMaxima}.
(Potential duplicates are ignored by using \Robject{dedup.on} during read extraction, to reduce the impact of read stacks on the constructed profile.)
For each selected window, \Rfunction{profileSites} records the coverage across the flanking regions as a function of the distance from the start of the window.
This is normalized by dividing by the maximum coverage across the interval for each window.
Coverage values are then averaged across selected windows to obtain an aggregated profile for each library.

<<>>=
all.bam <- c("h3ac.bam", "h3k4me2.bam", "es_1.bam")
collected <- list()
for (curbam in all.bam) {
    windowed <- windowCounts(curbam, spacing=50, width=50, 
                             param=dedup.on, filter=20)
...
The coverage profile around locally maximal windows is plotted below for each library.
We see that enrichment for the two histone marks is mostly contained within a 350 bp region around the maxima (dashed lines).
This suggests that a window size of \textasciitilde{}150 bp is ideal, given that directional extension by 100 bp has been performed on both sides of the peak.
...
This indicates that a smaller window size (\textless{} 50 bp) is probably adequate, consistent with sharp TF binding.
See the \Rfunction{wwhm} function for selection of a window width from the coverage profile.
...
This can be done manually setting the \Rcode{forward} slot in the \Rclass{readParam} object to \Rcode{TRUE} or \Rcode{FALSE}, to count only forward- or reverse-strand reads respectively.
...
A larger \Rcode{prior.count} of 5 is used to compute the average abundance.
This protects against inflated log-fold changes when the count for the window in the control sample is near zero.
Note that the global and local background estimates require less protection (\Rcode{prior.count=2}, by default) as they are derived from larger bins with more counts.
...
To remove composition biases in \Biocpkg{csaw}, reads are counted in large bins and the counts are used for normalization with the \Rfunction{normOffsets} wrapper function.
...
\paragraph{What does the TMM procedure do?}
...
Note that the \Rfunction{normOffsets} method skips the precision weighting step in the TMM method.
...
\paragraph{Why are large bins used?}
...
The variance of the fold change distribution is also higher for low counts, which reduces the effectiveness of the trimming procedure during normalization. 
...
This is valid \emph{provided that the library sizes are the same between the two sets of counts}, as the factors are interpreted with respect to the library sizes.
With \Biocpkg{csaw}, separate calls to \Rfunction{windowCounts} with the same \Rclass{readParam} object will always yield the same library sizes in \Rcode{totals}.

\paragraph{What is \Rfunction{normOffsets} actually doing?}
The \Rfunction{normOffsets} call computes normalization factors from \Robject{binned} and returns a modified version of \Robject{filtered.data}.
...
The binned TMM strategy described above requires the user to supply a bin size. 
...
<<normplot, fig.width=8, fig.height=3>>=
...
<<>>=
filtered.me <- normOffsets(filtered.me, se.out=TRUE)
...
filtered.ac <- normOffsets(filtered.ac, se.out=TRUE)
ac.eff <- filtered.ac$norm.factors
ac.eff
@

\paragraph{Why are we using windows now, not bins?}
Unlike with composition biases, normalization for efficiency biases is performed on window counts instead of bin counts.
...
In general, the estimated normalization factors are usually robust to the choice of window size. 

\paragraph{What happens to genuine biological differences?}
...
\paragraph{How does the choice of filter affect the normalization?}
Retaining too many windows will include background regions, which will obviously interfere with calculation of normalization factors from binding sites.
Moreover, the computed factors will not be applicable to the background regions as they are not subject to any efficiency biases in the first place.
Attempting to use the factors on windows from background regions will result in incorrect normalization and misleading log-fold changes.
Thus, it is usually better to err on the side of caution and filter more aggressively to ensure that background regions are not retained in downstream analyses.
...
<<>>=
me.comp <- normOffsets(me.bin, se.out=FALSE)
me.comp
ac.comp <- normOffsets(ac.bin, se.out=FALSE)
ac.comp
@
...
<<methplot,fig.width1=0, fig.height=5>>=
par(mfrow=c(1,2))
...
<<>>=
endog.data <- normOffsets(spike.data, se.out=endog.data)
@

\paragraph{What is the \Rfunction{normOffsets} call doing?}
The spike-in normalization factors are computed using high-abundance regions in \Robject{spike.data}, and returned in a modified copy of \Robject{endog.data}.
The new \Robject{endog.data} can then be used for further analysis of the endogenous windows.
The normalization factors can also be extracted from \Rcode{endog.data\$norm.factors}, as previously described.
...
\paragraph{How does this compare to the other scaling methods?}
...
ac.demo2 <- normOffsets(ac.demo2, type="loess", se.out=TRUE)
...
<<trendplot1, figt.width=10, fig.height=5>>=
par(mfrow=c(1,2))

# MA plot without normalization.
ac.y <- asDGEList(as.demo2)
...
\paragraph{Where is the offset matrix stored?}
...
If the offsets are available, they will generally override the normalization factors from the scaling methods in the downstream \Biocpkg{edgeR} analysis.

\paragraph{What's the relationship with normalizing for efficiency bias?}
Loess normalization of trended biases is quite similar to scaling normalization for efficiency biases in Section~\ref{sec:eff_norm}.
...
Loess normalization involves a slightly stronger assumption of non-DB at each abundance, not just across all bound regions.
...
\paragraph{Why do we need to filter?}
...
\paragraph{Why are we using 2 kbp windows?}
...
<<shrinkx,fig.width=10,fig.height=5>>=
par(mfrow=c(1,2))
o <- order(y$AveLogCPM)
...
<<>>=
cons <- consolidateSizes(list(ac.small, ac.large), 
                         list(mock.small, mock.large), 
...
<<>>=
cons.broad <- consolidateSizes(list(ac.small, ac.large), 
                               list(mock.small, mock.large), 
