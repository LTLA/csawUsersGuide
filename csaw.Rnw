...
<<>>=
spike.facs <- normalize(spike.data, lib.sizes=data$totals)
@
...
data.frame(DB.up=sum(is.sig & has.up & !has.down),
    DB.down=sum(is.sig & has.up & !has.down),
    DB.both=sum(is.sig & has.up & has.down))
@
...
This is simpler and allows quantification of the magnitude of the change, but will not capture complex DB between conditions.
