...
In \edger{}, the log-transformed overall mean is defined as the average abundance and is computed with the \code{aveLogCPM} function.
...
For demonstration purposes, an arbitrary threshold of 2 is used here to filter the window abundances. 
