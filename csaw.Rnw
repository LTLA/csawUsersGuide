...
In the example below, the weight assigned to the top window is equal to twice the number of other windows in each cluster.
...
weights[tab.ave$best] <- 2*(freq[merged$id[tab.ave$best]] - 1)
weights <- pmax(weights, 1)
tabcom.w <- combineTests(merged$id, results$table, weights=weights)
head(tabcom.w)
@

The exact value of the weight for the top window is rather arbitrary.
Greater weight represents a stronger belief that the DB event occurs at the most abundant window.
In practical terms, the doubling used to define \code{weights} means that the combined $p$-value of the cluster will not be more than 50\% larger than the $p$-value of the top window.
This conservativeness is an acceptable cost for considering DB events elsewhere in the cluster.
