...
%%
% Alternatively, peak-calling programs like MACS \cite{zhang2008} can be used
...
scores provided by the aligner. The \subread{} aligner \cite{liao2013} 
...
the fast loess algorithm \cite{ballman2004} with adaptation to count data. It
...
the genome is genuinely bound. Only the top proportion of high-abundance
windows are then retained. This approach is simple and has the practical
advantage of maintaining a constant number of windows for the downstream
analysis. However, it may not adapt well to different datasets where the
relative proportion of binding can vary. The biological or technical 
interpretation of this filter is also unclear.
...
properly modelled in the QL framework. Alternative normalization methods may be
necessary to resolve this issue (Section~\ref{sec:normbatch}).
