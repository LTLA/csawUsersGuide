...
The average abundances of each window must be scaled up to be comparable to those of the bins.
...
relative.width <- getWidths(data)/median(getWidths(binned))
win.ab <- scaledAverage(asDGEList(data), scale=relative.width)
@
...
The width of the window is also subtracted to obtain the effective size of the neighbourhood, to reflect the change in the counts.
...
Relative widths are then supplied to \code{scaledAverage}.
...
<<>>=
priorc <- 5
neighbour.ab <- scaledAverage(ny, prior.count=priorc)
win.ab <- scaledAverage(asDGEList(data), prior.count=priorc,
    scale=getWidths(data)/neighbour.width)
@
