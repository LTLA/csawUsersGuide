...
for (curbam in all.bam) {
	...
	collected[[curbam]] <- profileSites(bam, rowData(windowed)[maxed], 
        param=dedup.on, weight=1/rwsms[maxed])
}
plot(collected[[1]], type="l", col="blue", xlim=c(0, 5000), lwd=2, 
    xlab="Window size", ylab="Relative coverage")
lines(collected[[2]], col="darkgreen", lwd=2)
lines(collected[[3]], col="black", lwd=2)
@
...
The relative coverage represents the additional counts that can be obtained by expanding the region.
...
An alternative filter involves identifying local maxima in the read density across the genome.
This can be achieved using the \code{findMaxima} function.
The code below will determine if each window has the highest average abundance within 1 kbp on either side of that window.
