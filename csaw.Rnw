...
<<>>=
in.demo <- windowCounts(c(bam.files, "input.bam"), ext=frag.len)
chip <- in.demo[,1:4]
control <- in.demo[,5]
@
...
In addition, the filter is unlike the others in that it does not operate on the abundance of the windows. 
...
Thus, it may be prudent to apply this filter before the multiplicity correction but after DB testing.
...
This can be done by supplying a ``reasonable'' value for the NB dispersion during GLM fitting.
A value of 0.05 - 0.1 may be appropriate, based on past experience.
DB windows can then be identified using the likelihood ratio test.
...
This means that duplicates may need to be removed to protect against spurious differences.
...
<<>>=
maxed.new <- maxed[keep.simple]
maxed.data <- filtered.data[maxed.new,]
maxed.res <- results[maxed.new,]
@
