...
Complex DB is often present if both \code{up} and \code{down} are substantially non-zero (opposing changes within the region) or if the total number of windows is much larger than either number (interval of constant binding adjacent to the DB interval).
...
The same function can also be used to overlap windows of varying size to pre-specified regions.
...
tabbroad$best.start <- start(rowRanges(data))[tab.best.broad$best]
...
tabbroad.w <- combineOverlaps(olap, results$table, relation.weights=broad.weights) 
@
...
These commands are stored in a Bash file, accessible at:
